            ...
            for (step in 1..maxSteps) {
                ...
                if (debugMode && screenshotBase64 != null) {
                    try {
                        ...
                        val dir = context.getExternalFilesDir("debug_screenshots")
                        val file = java.io.File(dir, "step_${step}.jpg")
                ...
                val action = try {
                    ...
                    val totalMs = System.currentTimeMillis() - stepStartTime
                    val errorMsg = "Invalid action format: ${e.message}"
                    Log.e(TAG, "Step $step: $errorMsg")
                    pendingActionFeedback = errorMsg
                    _stepResults.emit(
                        StepResult(
                            step = step,
                ...
                // Feed action result back to LLM in the next iteration
                pendingActionFeedback = if (stepResult.success) {
                    String.format(LlmPrompts.agentFeedback("action_success"), stepResult.detail)
                } else {
                    String.format(LlmPrompts.agentFeedback("action_failed"), stepResult.detail)
                }

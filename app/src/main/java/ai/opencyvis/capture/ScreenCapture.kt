...
object ScreenCapture {
    ...
    fun captureJpegBytes(displayId: Int = 0, virtualDisplayBitmap: Bitmap? = null): ByteArray? {
        val applyBezel = (displayId != 0)
        if (virtualDisplayBitmap != null) {
            return try {
                ImageUtil.processScreenshotToBytes(virtualDisplayBitmap, applyBezel = applyBezel)
            } finally {
                if (!virtualDisplayBitmap.isRecycled) virtualDisplayBitmap.recycle()
            }
        }
        val bytes = backend.captureScreen(displayId, 540, 85) ?: return null
        
        // If captured from backend (e.g. physical screen or remote VD), 
        // we still want to apply the status bar mask to guide the LLM if it's a VD.
        return try {
            val bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.size)
            if (bitmap != null) {
                ImageUtil.processScreenshotToBytes(bitmap, applyBezel = applyBezel)
            } else {
                bytes
            }
        } catch (e: Exception) {

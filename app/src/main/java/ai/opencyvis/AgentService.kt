...
class AgentService : Service() {
    ...
    fun startAgent(instruction: String) {
        ...
        scope.launch {
            ...
            // 检查配置：是否使用物理显示器 (Display 0)
            val usePhysicalDisplay = config.usePhysicalDisplay

            var vdm: VirtualDisplayManager? = null
            var displayId = 0
            var displaySize: Point? = null

            if (usePhysicalDisplay) {
                Log.i(TAG, "Using physical display (Display 0) for agent task")
                val dm = resources.displayMetrics
                displaySize = Point(dm.widthPixels, dm.heightPixels)
                // 立即返回桌面，营造“Agent正在工作”的视觉效果
    ...
    private fun refreshControlledTasksFromVd(reason: String) {
        ...
        // Use backend (shell uid) for task queries
        val b = backend ?: return
        val displayToQuery = vdm.displayId
        val taskId = b.getTopTaskIdOnDisplay(displayToQuery, packageName)
        if (taskId <= 0) {
